...
RSpec.describe Location do
  ...
  describe 'columns in database' do
    ...
    it 'does not have a column called color' do
      expect(Location.method_defined?(:color)).to be false
    end

    it 'has a column called flavor' do
      expect(location).to respond_to(:flavor)
    end

    it 'has a column called price' do
      expect(location).to respond_to(:price)
    end
  end
end
